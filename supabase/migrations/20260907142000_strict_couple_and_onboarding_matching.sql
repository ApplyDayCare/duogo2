-- Migration: Strict couple and onboarding matching rules
-- Ensures that:
-- 1. Users who have not completed onboarding or quiz NEVER receive candidate matches.
-- 2. Couple users who have not linked both partners NEVER receive candidate matches.
-- 3. Incomplete couples (missing partner_b or partner quiz) are NEVER suggested to anyone.
-- 4. Solo users only match with verified solo users; couples only match with verified couples.

CREATE OR REPLACE FUNCTION public.is_verified_couple_member(_user_id uuid)
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  SELECT EXISTS (
    ...
    WHERE (c.partner_a_id = _user_id OR c.partner_b_id = _user_id)
...
GRANT EXECUTE ON FUNCTION public.is_verified_couple_member(uuid) TO authenticated;
